...
_DEFAULT_JSON = os.path.join(_THIS_DIR, '..', 'sai_api_attributes.json')
...
class SaiApiTestBase(ThriftInterface):
    ...
    Sub-class and set class attributes to customise:
        ...
        json_path    -- path to sai_api_attributes.json
                        (default: ../sai_api_attributes.json relative to this file)
    ...
    def _load_json(self):
        path = os.path.normpath(
            os.path.join(_THIS_DIR, self.json_path)
    ...
    def _exec_create(self, fn_name: str, attributes: dict, ctx: dict,
        ...
        # For create, a leading _entry param means caller must supply the key
    ...
    def _exec_get(self, fn_name: str, attributes: dict, ctx: dict,
        ...
        if key_param:
            if key_param.endswith('_entry'):
                ...
                return
            if ctx['oid'] is None:
                self._record(obj_type, fn_name, 'SKIP: no OID available (create skipped/failed)')
                return
            pos_args = [ctx['oid']]
...
class SaiApiTest(SaiApiTestBase):
    ...
    Run with PTF::
        ...
        ptf --test-dir ptf sai_api_test.SaiApiTest \\
            --interface 0@<iface> \\
